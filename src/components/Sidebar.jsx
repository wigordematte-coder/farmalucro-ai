...
import { LayoutDashboard, Package, FileUp, BarChart3, Bot, Tag, Megaphone, FileText, Settings, X } from 'lucide-react';
import { NAV_ITEMS } from '@/lib/constants';
import { usePharmacy } from '@/lib/pharmacyContext';
import { cn } from '@/lib/utils';

const ICONS = { LayoutDashboard, Package, FileUp, BarChart3, Bot, Tag, Megaphone, FileText, Settings };
          ...
          <div className="px-3 py-2.5 rounded-xl bg-white/5">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={cn(
                ...
                settings?.subscription_status === 'active' ? 'bg-accent text-white' :
                ...
                {settings?.subscription_status === 'active' ? 'Ativa' :
                 ...
                 settings?.subscription_status === 'expired' ? 'Vencida' : 'Trial'}
              </span>
            </div>
            <p className="text-[11px] text-white/50">{settings?.subscription_plan || 'Plano Mensal'}</p>
          </div>
        </div>
      </aside>
    </>
