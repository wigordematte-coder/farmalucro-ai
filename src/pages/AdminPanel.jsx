...
export default function AdminPanel() {
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      <div>
        <h1 className="text-xl lg:text-2xl font-bold text-foreground">Painel Administrativo</h1>
        <p className="text-sm text-muted-foreground">Visão geral de assinaturas, receita e inadimplência</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard icon={Users} label="Total Assinantes" value={stats.total} color="primary" />
        <StatCard icon={CheckCircle2} label="Assinaturas Ativas" value={stats.active} color="accent" />
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
          <Receipt className="w-5 h-5 text-accent" /> Últimos Pagamentos Recebidos
        </h3>
        {stats.paidPayments.length === 0 ? (
          <div className="py-8 text-center text-sm text-muted-foreground">Nenhum pagamento recebido ainda.</div>
        ) : (
          <div className="overflow-x-auto -mx-5 px-5">
            <table className="w-full text-sm">
              <thead>
